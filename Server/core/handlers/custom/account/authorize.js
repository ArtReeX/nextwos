...
function authorize(mysql, data, callback) {
    ...
    mysql.getConnection(function (error, conn) {
        ...
        if (error) {
            ...
            conn.query("SELECT users.id, users.password, users.password_salt  FROM users WHERE users.username='" + String(data.username) + "' LIMIT 1", function (error_authorize, result_authorize) {
                ...
                if (error_authorize) {
                    ...
                    if (result_authorize.length) {
                        ...
                        if (bcrypt_module.hashSync(String(data.password), String(result_authorize[0].password_salt)) === String(result_authorize[0].password)) {
                            ...
                            // возврат результата
                            callback({ "error" : null, "data": { "result_authorize" : true, "identificator" : Number(result_authorize[0].id) } });
                            
                        } else {
                            
                            // возврат результата
                            callback({ "error" : { "type" : "incorrect_password" }, "data" : null});
                        ...
                        // возврат результата
                        callback({ "error" : { "type" : "user_not_exist" }, "data" : null});
