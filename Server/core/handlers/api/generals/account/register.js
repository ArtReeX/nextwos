...
function register(mysql, data, callback) {
    ...
    mysql.getConnection(function (error, conn) {
        ...
        if (error) {
            ...
            conn.query("SELECT general_accounts.id FROM general_accounts WHERE general_accounts.username='" + String(data.username) + "' LIMIT 1", function (error_verification, result_verification) {
                ...
                if (error_verification) {
                    ...
                    if (!result_verification.length) {
                        ...
                        // запись данных в БД
                        conn.query("INSERT INTO general_accounts (general_accounts.username, general_accounts.password, general_accounts.password_salt, general_accounts.first_name, general_accounts.second_name) VALUES ('" + String(data.username) + "', '" + String(password_md5) + "', '" + String(password_salt) + "', '" + String(data.first_name) + "', '" + String(data.second_name) + "')", function (error_register, result_register) {
                            ...
                            if (error_register) {
                                ...
                                callback({
                                    "error": null,
                                    "data": {
                                        "result_register": Boolean(result_register.affectedRows)
                                    }
                                });
